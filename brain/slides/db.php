...
-------------------------------------- */
    ...
    /*
        ...
        CREATE TABLE presentations (id INT(6) UNSIGNED AUTO_INCREMENT PRIMARY KEY, username VARCHAR(75) NOT NULL, source VARCHAR(20), dateMade VARCHAR(15));

        INSERT INTO presentations (username, source, dateMade) VALUES ('root', 'slides.md', 'date');
    */
    ...
    function updatePresentation($db, $source, $user, $id)
    {
        try
        {
            $query = "UPDATE presentations SET source=:c, dateMade = :d WHERE id = :i;";
            $statement = $db->prepare($query);
           // $statement->bindvalue(':u', $user);
            $statement->bindvalue(':c', $source);
    ...
    function newPresentation($db, $source, $user)
    {
        try
        {
            $query = "INSERT INTO presentations (username, source, dateMade) VALUES (:u, :c, :d);";
            $statement = $db->prepare($query);
           // $statement->bindvalue(':u', $user);
            $statement->bindvalue(':c', $source);
            $statement->bindvalue(':u', $user);
    ...
    function deletePresentation($db, $id)
    {
        try
        {
            $query = "DELETE FROM presentations WHERE id = :i;";
            $statement = $db->prepare($query);
            $statement->bindvalue(':i', $id);
            $statement->execute();
            $statement->closeCursor();
